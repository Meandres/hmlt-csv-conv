...
| tableau { prevLen=-1; len=0; printf("\n"); }
...
ligne_entetes: DEBLIG liste_entetes FINLIG {
	printf("\n");
	if(prevLen==-1) { prevLen=len; len=0; }
	else{
		if(len!=prevLen) { yyerror("Les lignes n'ont pas le même nombre de cellules"); }
			else{ prevLen=len; len=0; }
		}
	}
liste_entetes: entete { printf("%s", $1); }
| liste_entetes entete { printf(";%s", $2); }
entete: DEBCELENT CONTENU FINCELENT { $$=$2; len++; }
